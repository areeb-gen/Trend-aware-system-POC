    ...
    if st.button("Find your thing", disabled=not query, type="primary"):
        ...
        if tool_calls:
            ...
            with cols[0]:
                ...
                rag = tool_calls.get("rag", {})
                st.markdown(f"**RAG** — {'on' if rag.get('enabled') else 'off'} ({rag.get('chunk_count', 0)} chunks retrieved)")
                if rag.get("enabled") and rag.get("chunks"):
                    for chunk in rag["chunks"]:
                        st.caption(f"[{chunk.get('similarity', 0):.2f}] {chunk.get('title', '')} · {chunk.get('source', '')} · {chunk.get('brief_date', '')}")
